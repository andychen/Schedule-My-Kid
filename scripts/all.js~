 ...
 *  ExhibitBundle 
 *  EnglishLocalization 
 *  EnglishLocalizationBundle
 *  TimegridAPI
 */

/*==================================================
 *  Simile Ajax API - SimileAjaxAPI
 *==================================================
 */
...
try{SimileAjax.jQuery.ajax({type:"POST",url:SimileAjax.RemoteLog.url,data:A});}catch(e){}
}};
...
(function() {
    ...
    var loadMe = function() {
	    ...
	    var url = SimileAjax.findScript(document, "/all.js");
//            var url = SimileAjax.findScript(document, "/exhibit-api.js");
            ...
            Exhibit.urlPrefix = url.substr(0, url.indexOf("all.js"));
...
Exhibit.TsvCsvImporter.parseTsvCsv=function(K,J,D,A){var E=Exhibit.TsvCsvImporter.formatType(J);
var M=K.split("\n");
var C;
var B=D!=null;
var G;
...
var L={"properties":I,"items":H};
return L;
};
Exhibit.TsvCsvImporter.formatType=function(B){var A="";
if(B=="tab-separated-values"||B=="tsv"){A=A.concat("\t");
}else{if(B=="comma-separated-values"||B=="csv"){A=A.concat(",");
...
Exhibit.SliderFacet.createFromDOM=function(F,E,A){var H=Exhibit.getConfigurationFromDOM(F);
var A=Exhibit.UIContext.createFromDOM(F,A);
var G=new Exhibit.SliderFacet(E!=null?E:F,A);
Exhibit.SettingsUtilities.collectSettingsFromDOM(F,Exhibit.SliderFacet._settingsSpecs,G._settings);
try{var B=Exhibit.getAttribute(F,"expression");
if(B!=null&&B.length>0){G._expression=Exhibit.ExpressionParser.parse(B);
}var C=Exhibit.getAttribute(F,"showMissing");
if(C!=null&&C.length>0){G._showMissing=(C=="true");
}else{G._showMissing=true;
}}catch(D){SimileAjax.Debug.exception(D,"SliderFacet: Error processing configuration of slider facet");
}Exhibit.SliderFacet._configure(G,H);
G._initializeUI();
...
/*==================================================
 *  English localization - EnglishLocalization
